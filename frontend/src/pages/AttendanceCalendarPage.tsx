...
export default function AttendanceCalendarPage() {
  ...
  const { list: attendanceList, loading } = useSelector((state: RootState) => state.attendance)
 ...
 const recordsByDate = useMemo(() => {
  const map: Record<
    string,
    Record<number, { status: AttendanceStatus; note?: string; markedBy?: { id: number; name: string } }>
  > = {}
...
const getAttendanceForDate = (date: Date): DayAttendance & { markedBy: Record<string, any> } => {
 ...
 const dayStats = useMemo(() => {
  const stats: Record<string, { absent: number; late: number; excused: number; unexcused: number; total: number }> = {}
...
const selectedDateStats = useMemo(() => {
  ...
  return {
    ...
    stats: {
      ...
      unexcused: membersWithStatus.filter((m) => m.status === "unexcused").length,
      ...
      <div className="space-y-4 animate-fadeIn">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Xem lịch sử điểm danh theo tháng</p>
          </div>
          <Button
        ...
        <Card className="border-2 shadow-lg rounded-2xl">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>Click vào ngày để xem chi tiết • Vuốt để chuyển tháng</CardDescription>
                </div>
              </div>
          ...
          <CardContent onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
            <div className={`transition-opacity duration-150 ${isTransitioning ? "opacity-50" : "opacity-100"}`}>
              <div className="grid grid-cols-7 gap-2">
                ...
                {daysInMonth.map((day) => {
                  ...
                  const hasIssues = stats.absent > 0 || stats.late > 0 || stats.excused > 0 || stats.unexcused > 0
                      ...
                      <div className="flex flex-col h-full">
                        ...
                        {hasAttendance && (
                          <div className="flex gap-0.5 justify-center mt-1">
                            {stats.absent > 0 && <div className="h-1 w-1 rounded-full bg-red-500" />}
                            {stats.late > 0 && <div className="h-1 w-1 rounded-full bg-orange-500" />}
                            {stats.excused > 0 && <div className="h-1 w-1 rounded-full bg-blue-500" />}
                            {stats.unexcused > 0 && <div className="h-1 w-1 rounded-full bg-red-700" />}
                          </div>
                        )}
                      </div>
                    </button>
              ...
              <>
                <DialogHeader>
                  <DialogTitle className="text-2xl">
                    Điểm danh ngày {format(selectedDateStats.date, "dd/MM/yyyy", { locale: vi })}
                  </DialogTitle>
                  <DialogDescription>{format(selectedDateStats.date, "EEEE", { locale: vi })}</DialogDescription>
                </DialogHeader>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 my-4">
                  <div className="p-3 rounded-xl bg-gray-50 border-2 border-gray-200">
                    ...
                    <div className="text-2xl font-bold text-gray-900">{selectedDateStats.stats.total}</div>
                  </div>
                  <div className="p-3 rounded-xl bg-emerald-50 border-2 border-emerald-200">
                    ...
                    <div className="text-2xl font-bold text-emerald-700">{selectedDateStats.stats.present}</div>
                  </div>
                  <div className="p-3 rounded-xl bg-red-50 border-2 border-red-200">
                    ...
                    <div className="text-2xl font-bold text-red-700">{selectedDateStats.stats.absent}</div>
                  </div>
                  <div className="p-3 rounded-xl bg-orange-50 border-2 border-orange-200">
                    ...
                    <div className="text-2xl font-bold text-orange-700">{selectedDateStats.stats.late}</div>
                  </div>
                  <div className="p-3 rounded-xl bg-blue-50 border-2 border-blue-200">
                    ...
                    <div className="text-2xl font-bold text-blue-700">{selectedDateStats.stats.excused}</div>
                  </div>
                  <div className="p-3 rounded-xl bg-red-50 border-2 border-red-200">
                    <div className="flex items-center gap-2 text-red-600 text-sm mb-1">
                    ...
                    <div className="text-2xl font-bold text-red-700">{selectedDateStats.stats.unexcused}</div>
                  </div>
                </div>
                <Separator />
                <Tabs defaultValue="all" className="mt-4">
                  ...
                  <TabsContent value="all" className="space-y-3 mt-4">
                    ...
                    {selectedDateStats.members.filter((m) => m.status !== "present").length === 0 && <EmptyState />}
                  </TabsContent>
                  <TabsContent value="absent" className="space-y-3 mt-4">
                    ...
                    {selectedDateStats.members.filter((m) => m.status === "absent").length === 0 && (
                      <EmptyState message="Không có ai vắng" />
                    )}
                  </TabsContent>
                  <TabsContent value="late" className="space-y-3 mt-4">
                    ...
                    {selectedDateStats.members.filter((m) => m.status === "late").length === 0 && (
                  ...
                  <TabsContent value="excused" className="space-y-3 mt-4">
                    ...
                    {selectedDateStats.members.filter((m) => m.status === "excused").length === 0 && (
                  ...
                  <TabsContent value="unexcused" className="space-y-3 mt-4">
                    ...
                    {selectedDateStats.members.filter((m) => m.status === "unexcused").length === 0 && (
    ...
    <div className="flex items-start gap-3 p-3 rounded-xl border-2 bg-white hover:shadow-md transition-all duration-300">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <Badge className={`${getStatusColor(member.status)} border-2 rounded-full text-xs`}>
            {getStatusLabel(member.status)}
          </Badge>
        </div>
        {member.markedBy && (
          <p className="text-xs text-gray-500">
            Người điểm danh: <span className="font-medium text-gray-700">{member.markedBy.name}</span>
          </p>
        )}
        {member.note && <p className="text-sm text-gray-500 mt-1 italic">Ghi chú: {member.note}</p>}
      </div>
    </div>
  )
}
...
function EmptyState({ message = "Tất cả đoàn sinh đều có mặt!" }: { message?: string }) {
  return (
