...
import { AlertTriangle, ChevronRight, X, UserX, Info, ExternalLink } from "lucide-react";
...
export const RiskCenterSection: React.FC<RiskCenterSectionProps> = ({
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-rose-100 shadow-sm mb-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900">🚨 Trung tâm Cảnh báo Nguy cơ (Risk Center)</h2>
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 px-3.5 py-2 rounded-xl border border-rose-200 transition-colors"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        ...
        <div
          ...
          className="bg-gradient-to-br from-rose-50 to-red-50 p-4 rounded-xl border border-rose-200 hover:border-rose-400 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md"
        >
          ...
          <p className="text-[11px] text-rose-700/80 mt-1">Cần liên hệ gia đình gấp</p>
        </div>
        ...
        <div
          ...
          className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 rounded-xl border border-amber-200 hover:border-amber-400 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md"
        >
          <div className="flex items-center justify-between">
          ...
          <p className="text-[11px] text-amber-700/80 mt-1">Cần nhắc nhở và theo dõi thêm</p>
        </div>
        ...
        <div
          ...
          className="bg-gradient-to-br from-yellow-50 to-amber-50/40 p-4 rounded-xl border border-yellow-200 hover:border-yellow-400 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-black uppercase text-yellow-800 tracking-wider flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-yellow-500"></span>
              🟡 NGUY CƠ THẤP (LOW)
            </span>
            <span className="text-2xl font-black text-yellow-900">{lowRisks.length}</span>
          </div>
          <p className="text-[11px] text-yellow-700/80 mt-1">Có dấu hiệu giảm sút nhẹ</p>
        </div>
      </div>
      ...
      {riskMembers.length > 0 && (
        <div className="border border-slate-100 rounded-xl overflow-hidden bg-slate-50/50">
          <div className="px-4 py-2.5 text-xs font-semibold text-slate-500 bg-slate-100 border-b border-slate-200 flex justify-between">
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500">({m.branch})</span>
                    <span
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredMembers.map((m) => (
                      <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 font-medium text-slate-700">{m.branch}</td>
