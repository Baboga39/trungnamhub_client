  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm mb-6">
      <div className="flex items-center gap-2.5 mb-5">
        <div className="p-2.5 bg-purple-100/80 rounded-2xl text-purple-700">
          <Flame className="w-5 h-5" />
        </div>
        <div>
          <h2 className="text-lg font-black text-slate-900 tracking-tight">🏕️ Tham Gia Phong Trào Ngoại Khóa</h2>
          <p className="text-xs text-slate-500 font-medium">Tỷ lệ đoàn sinh tham gia chuỗi hoạt động trong Quý</p>
        </div>
      </div>
        ...
        <div className="space-y-3.5">
          {activities.map((act) => {
            ...
            return (
              <div
                key={act.id}
                className="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/70 hover:border-purple-300 transition-all duration-200 shadow-2xs hover:shadow-xs"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {dateStr && (
                      <span className="text-xs text-slate-400 font-medium flex items-center gap-1 ml-1.5">
